...
なお、参考までに私のプロジェクトでは主にライブラリを使用していました。(モジュールの正式名称での記載は省略)
...
このステップのポイントとして、リリースノートの情報だけで自プロダクトがModule Systemに移行可能だと思い込んで作業を終了してしまうのは危険です。 **自分のプロジェクトがNamed Moduleとしてマイグレーション可能か(ここで言うStep 4)** まで確認した方が良いでしょう。

「何をもってJava9に対応したか」はライブラリ毎に異なるとは思いますが、おそらく「モジュール単体としてはAutomatic Moduleとして使用可能」な位だろうと推察しています。そのため、当然のことながら **ライブラリ間の競合は加味されていない** と考えた方が自然です。
実際のアプリケーションでは様々なライブラリを組み合わせて構築していますから、module pathによるクラスの読み込み方が
