...
（リンカはオブジェクトファイルを１つの実行ファイルやDLLにまとめるプログラム）で、多くのOSで採用されています。

中盤はその開発の経緯や、開発時の経験談を伺うことができました。

リンカを作るか、コンパイラを作るかでリンカを選択したこと、
リンカを作ると決めてから、バイナリを印刷して読みふけって独学したこと、
一度作成したリンカを「一回書き直したい」と提案し、賛成派と反対派でチームを真っ二つに割ったこと、
それでも自分が勇気を持って開発を推進していく姿勢、
まさにスーパープログラマの人生の断片を覗かせていただきました。

また、リンカを実装する過程でのプログラミングに対する考え方も

* 「（gnu goldは）一見きれいなアーキテクチャにも見えるけど、[lld](https://lld.llvm.org/) はもっとシンプルに処理をするようにしています」
* 「似たようなコードでも、実は微妙に違うから、変に共通化しない方がいい」
* 「こだわってもしょうがないところは捨てる」
* 「（コードの書き直しに）反対している人って、だいたいコード書いていない人だったんですよね」
* 「データ構造が重要。コードはデータ構造が決まれば自然と書ける」
* 「（コードを書き直す所の勘所は）勘」

**原理主義に陥るのではなく、シンプルに考えてコードを書き、強い勇気を持って、プロダクトを守り、推進していくのが大切だ** 、というメッセージと理解しました。
